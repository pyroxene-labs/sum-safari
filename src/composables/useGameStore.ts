...
export function useGameStore() {
  ...
  function nextProblem(): void {
    ...
    currentProblem.value = generateProblem(session.value.settings.difficulty, session.value.settings.operators);
  ...
  const scoreSummary = computed<ScoreSummary | null>(() => {
    ...
    return calculateScoreSummary(session.value.results, session.value.hintTimestamps, session.value.settings.difficulty, sessionDurationMs);
  });
