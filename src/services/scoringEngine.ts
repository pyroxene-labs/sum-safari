...
import { type ProblemResult, type ScoreSummary, type Difficulty, SCORING } from "@/types";

/**
 * Calculate points for a single problem result
 */
export function calculateProblemPoints(result: ProblemResult, hintUsed: boolean): number {
  if (result.skipped) {
 ...
 */
export function calculateExcessHints(hintTimestamps: number[], sessionDurationMs: number): number {
  if (hintTimestamps.length === 0) {
    return 0;
  }

  const sessionMinutes = Math.max(1, sessionDurationMs / 60000);
  const allowedHints = Math.floor(sessionMinutes * SCORING.EXCESS_HINT_THRESHOLD);
 ...
 */
export function calculateScoreSummary(results: ProblemResult[], hintTimestamps: number[], difficulty: Difficulty, sessionDurationMs: number): ScoreSummary {
  let basePoints = 0;
  ...
  const subtotal = Math.max(0, basePoints + timeBonusPoints - hintPenalty - excessHintPenalty);
...
export function getScoreBreakdownText(summary: ScoreSummary): string[] {
  ...
  lines.push(`Correct Answers: ${summary.correctCount} (+${summary.basePoints} base points)`);

  if (summary.skippedCount > 0) {
    lines.push(`Skipped: ${summary.skippedCount} (-${summary.skippedCount * SCORING.SKIP_PENALTY} points)`);
  }
